...
const extractRegionFromConfigFile = (params, callback) => {
  let sharedAWSConfigFilePath = process.env.HOME ? process.env.HOME : os.homedir();
  sharedAWSConfigFilePath += `/.aws/config`;
  const data = ini.parse(sharedAWSConfigFilePath);
  const profile = params.profile ? params.profile : 'default';
  // some config files use this format:
  if (data[`profile ${profile}`]) {
    return callback( data[`profile ${profile}`].region);
  } else {
    return callback( data[profile].region);
  }
};
...
// 4. 'us-east-1'
const initRegion = (params, callback) => {
  const region = params.region ? params.region : process.env.AWS_DEFAULT_REGION;
  if (region) {
    return callback(new AWS.CloudWatchLogs({ region }));
  }
  // otherwise try to get from config file:
  try{
    extractRegionFromConfigFile(params, (region) => {
      return callback(new AWS.CloudWatchLogs({ region }));
    });
  } catch (exc) {
    console.log('Falling back to region us-east-1......');
    return callback(new AWS.CloudWatchLogs({ region: fallbackAWSRegion }));
  }
};

module.exports = (argv, callback) => {
  initCustomCredentials(argv);
  initRegion(argv, callback);
};
